...
class HeroItem(MDHeroFrom):
    ...
    def on_release(self, *args):
        def switch_screen(*args):
            ...
            screen_b.heroes_to[0].tag = self.tag
...
class ScreenB(MDScreen):
    heroes_to = ObjectProperty()  # Use ObjectProperty

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.heroes_to = [MDHeroTo(size_hint=(1, None), height="220dp", pos_hint={"top": 1})]
        self.add_widget(self.heroes_to[0])
    ...
    def switch_to_screen_a(self, *args):
        print('b', self.heroes_to[0].tag)
        self.manager.current = "screen A"
        self.manager.current_heroes = [self.heroes_to[0].tag]
